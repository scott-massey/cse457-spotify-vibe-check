...
const Radar = ({ data, featuresSummary, activePlaylist, loading }) => {
  ...
  const renderChart = (svg) => {
    ...
    radarWidth = Math.min(700, window.innerWidth - 10) - margin.left - margin.right,
    radarHeight = Math.min(width, window.innerHeight - margin.top - margin.bottom - 20);
    ...
    if(featuresSummary){
        var allAxis = (featuresSummary.map(function(i, j){return i.key})),	//Names of each axis
    ...
    //Append a g element		
    var g = svg.append("g")
    .attr("transform", "translate(" + (cfg.w/2 + cfg.margin.left) + "," + (cfg.h/2 + 50) + ")");
...
//Append the labels at each axis
	...
	var radarLine = d3.lineRadial()
        .radius(function(d) { return scaleValue(d); })
...
//Append the backgrounds	
    blobWrapper
        ...
        .attr("d", function(d,i) { return radarLine(testData); })
...
//Create the outlines	
    ...
    blobWrapper.selectAll(".radarCircle")
        ...
        .attr("cx", function(d,i){ return scaleValue(d) * Math.cos(angleSlice*i - Math.PI/2); })
        .attr("cy", function(d,i){ return scaleValue(d) * Math.sin(angleSlice*i - Math.PI/2); })
        .style("fill", "red")
