...
const ref = useD3(
    (svg) => {
      ...
      // SVG drawing area
        ...
        svg.select(".plot-area").append("text").attr("class","avgLabel").style("opacity",0).attr("x",'50%').attr("y",50);
        
        // Scales and axes
        const x = d3.scaleLinear()
            .range([0, width])
            .domain((function(d) { return d.low; }, function(d) { return d.high; }));
    ...
    //Draw circles for average value
        circle.enter().append("circle")
            .attr("cx", function(d) { return width/2 + (100*d.avg); })
